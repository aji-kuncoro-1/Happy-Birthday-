...
function refreshRig(){
  // Mobile gets a slightly higher heart + tighter bow anchor so the entire
  // composition remains reachable on 360–430px portrait screens. Desktop and
  // tablet keep the original geometry unchanged.
  const mobile = window.matchMedia('(max-width: 430px)').matches;
  const gripX = W * (mobile ? 0.205 : 0.24);
  const gripY = H * (mobile ? 0.735 : 0.76);
  const heartX = W * 0.5;
  const heartY = H * (mobile ? 0.30 : 0.33);
  ...
  maxDraw = mobile
    ? Math.min(bR.height * 0.78, H * 0.145, 108)
    : Math.min(bR.height * 0.72, H * 0.16, 132);
  curDraw = 0;
}
...
function shotGeom(){
  ...
  const mobile = window.matchMedia('(max-width: 430px)').matches;
  const fallPx = Math.min(
    H * (mobile ? 0.24 : 0.26),
    Math.max(0, H - tcy - tRect.height * 0.4)
  );
  ...
  return {
    ...
    // Keep the mobile burst large enough to cover narrow portrait edges while
    // preserving the original desktop timing/scale.
    floodScale: ((distC * (mobile ? 1.22 : 1.12)) / 70),
    bloomScale: ((reach * (mobile ? 1.28 : 1.2)) / 30),
  };
}
...
function resize(){
  ...
  // On mobile browsers the visual viewport changes as the address bar expands
  // or collapses. Let CSS own the layout, but use the current viewport as a
  // fallback so the canvas never keeps a stale height.
  if (window.matchMedia('(max-width: 430px)').matches && window.visualViewport){
    H = Math.min(H, Math.max(1, Math.round(window.visualViewport.height)));
  }
...
window.visualViewport && window.visualViewport.addEventListener('resize', queueResize, { passive:true });
window.visualViewport && window.visualViewport.addEventListener('scroll', queueResize, { passive:true });
