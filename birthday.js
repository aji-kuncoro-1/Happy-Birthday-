...
/* --- impact sound ----------------------------------------------------------
   A tiny Web Audio effect is synthesized in-browser, so no external audio
   file is needed. The audio context is unlocked from the user's pointer/key
   gesture and the impact is triggered exactly when the arrow hits the heart. */
let audioCtx = null;
...
function playHeartBreakSound(){
  ...
  // Short glassy sparkle after the heart breaks.
  [740, 1047, 1480].forEach((freq, i) => {
    const chime = audioCtx.createOscillator();
    const chimeGain = audioCtx.createGain();
    chime.type = 'sine';
    chime.frequency.setValueAtTime(freq, now + 0.025 + i * 0.018);
    chimeGain.gain.setValueAtTime(0.0001, now + 0.025 + i * 0.018);
    chimeGain.gain.exponentialRampToValueAtTime(0.055, now + 0.04 + i * 0.018);
    chimeGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.42 + i * 0.03);
    chime.connect(chimeGain).connect(audioCtx.destination);
    chime.start(now + 0.025 + i * 0.018);
    chime.stop(now + 0.45 + i * 0.03);
  });
}
...
function buildFilm(m){
  ...
  t.add(() => { playHeartBreakSound(); cue('heart-impact'); }, 0.26)
   .add(burstHearts, 0.26)
